...
int encrypt_sector(size_t key_len, unsigned char *restrict key, unsigned char *restrict iv, unsigned char *restrict sector_in, unsigned char *restrict sector_out); // encrypts a sector of length SECTOR_LENGTH using the specified key and IV, storing the result in sector_out (which should also be of length SECTOR_LENGTH, ie. the IV is not prepended).  key_len is in BYTES
int decrypt_sector(size_t key_len, unsigned char *restrict key, unsigned char *restrict iv, unsigned char *restrict sector_in, unsigned char *restrict sector_out); // decrypts a sector of length SECTOR_LENGTH using the specified key and IV, storing the result in sector_out.  key_len is in BYTES
